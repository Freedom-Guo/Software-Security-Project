...
void Operation_analyze(char str[MAX],struct Operation* op);
bool Judge_operation(char str[MAX]);
...
int main(int argc, const char * argv[])
{
    ...
    while(f->func_next != NULL)
    {
        ...
        while(op->o_next!=NULL)
        {
            flag_op=1;
            op=op->o_next;
            strcpy(op_des,op->des);
            while(v->v_next!=NULL)
            {
                v=v->v_next;
                if(!strcmp(op_des,v->name))
                    des_type_flag=v->type_flag;
            }
            if(des_type_flag<5)
                flag_op=2;
        }
        while(m->m_next!=NULL)
        {
            ...
            while(b->buf_next!=NULL)
            {
                b=b->buf_next;
                if(!strcmp(b->name,str_des))
                    flag_m=2;
            }
        }
        if(flag_op==2 && flag_m==2)
            over_flow=2;
        else
            over_flow=3;
        ...
        if(over_flow==2)
        {
            printf("Possibility of arithmetic overflow in line %d caused by line %d:\n",m->row,op->row);
            Show(pf2,op->row,str_output);
            Show(pf2,m->row,str_output);
            printf("\n");
        }
        else
            printf("No arithmetic overflow!\n");
    }
    return 0;
}
...
void Operation_analyze(char str[MAX],struct Operation* op)
{
    ...
    {
        if(strstr(str,"*")!=NULL &&strstr(str,"malloc")==NULL && strstr(str,"short*")==NULL && strstr(str,"int*")==NULL && strstr(str,"long*")==NULL)
        {
            //乘法计算语句
            sscanf(str,"%[^=]",op->des);
            str=strstr(str,"=");
        ...
        {
            if(strstr(str,"*")!=NULL &&strstr(str,"malloc")!=NULL)
            {
                str=strstr(str,"(");
                str++;
                sscanf(str,"%[^*]",op->des);
                strcpy(op->sou_1,op->des);
                str=strstr(str,"*");
                str++;
                sscanf(str,"%[^;]",op->sou_2);
            }
...
struct Func* funcchain(FILE *pf)
{
    ...
    struct Operation *op_head,*op_tail,*op_new;
    ...
    while(!feof(pf))
    {
        ...
        if(Judge_body(buf_1))
        {
            ...
            op_head = (struct Operation*)malloc(sizeof(struct Operation));
            op_tail = op_head;
            op_tail->o_next = NULL;
            ...
            do
            {
                ...
                if(Judge_operation(buf_2))               //扫描有无运算语句
                {
                    op_new=(struct Operation*)malloc(sizeof(struct Operation));
                    op_new->row=row;
                    Operation_analyze(buf_2, op_new);
                    op_tail->o_next=op_new;
                    op_tail=op_new;
                    op_tail->o_next=NULL;
                }
            }
